            ...
            if (subcommand === "setup") {
                ...
                try {
                    ...
                    const logEmbed = createEmbed({
                        ...
                        { name: "Max Tickets Per User", value: maxTicketsPerUser.toString(), inline: true },
                        { name: "DM on Close", value: dmOnClose ? 'Enabled' : 'Disabled', inline: true },
                        {
                            name: "Moderator",
                    ...
                    if (logChannelId) {
                        ...
                        if (logChannel) {
                            await logChannel.send({ embeds: [logEmbed] }).catch(err => {
                                logger.warn('Failed to send log embed to log channel', {
                                    error: err.message,
                                    guildId: interaction.guildId,
                                    logChannelId
                                });
